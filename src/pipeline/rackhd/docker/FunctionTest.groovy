...
def runTest(String stack_type, String test_name, ArrayList<String> used_resources, Map manifest_dict, Map docker_build_ret_dict){
    ...
    try{
        lock(label:label_name,quantity:1){
            ...
            """
                    ...
                    String build_record_file = "$WORKSPACE" + "/" + docker_build_ret_dict["DOCKER_RECORD_STASH_PATH"]
                    rackhd_deployer.deploy(library_dir, log_dir, rackhd_dir, docker_tar_file) 
                    echo "rackhd_deployer.deploy done. huangqiang"
                    ...
                    echo "fit.configControlInterfaceIp(rackhd_dir) done."
                ...
                //    error("[ERROR] Failed to run test $test_name against $test_target with error: $error")
                    ...
                    echo "[DEBUG] fit.archiveLogsToTarget"
                    virtual_node.stopFetchLogs(library_dir, target_dir)
                    echo "[DEBUG] virtual_node.stopFetchLogs"
                    virtual_node.archiveLogsToTarget(target_dir)
                    echo "[DEBUG] virtual_node.archiveLogsToTarget"

                    // clean up rackhd and virtual nodes
                    ignore_failure = true
                    rackhd_deployer.cleanUp(library_dir, rackhd_dir, ignore_failure)
                    echo "[DEBUG]rackhd_deployer.cleanUp done huangqiang"
                    virtual_node.cleanUp(library_dir, ignore_failure)
                    echo "[DEBUG] virtual_node.cleanUp"
                }
            }
         }
    } finally{
         used_resources.remove(node_name)
         echo "[DEBUG] used_resources.remove"
    }
}
